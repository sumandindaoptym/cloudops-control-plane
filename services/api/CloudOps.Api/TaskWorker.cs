...
public class TaskWorker : BackgroundService
{
    ...
    public TaskWorker(ILogger<TaskWorker> logger, IMessageBus messageBus, IServiceProvider serviceProvider, IHubContext<TaskHub> hubContext)
    {
        _logger = logger;
    ...
    {
        _logger.LogInformation("TaskWorker started at: {time}", DateTimeOffset.Now);
    ...
    private async Task SendUpdate(Guid taskId, string status, string? message, int? percent = null)
    {
        try
        {
            await _hubContext.Clients.All.SendAsync("SendTaskUpdate", new
            {
                taskId,
                status,
                message,
